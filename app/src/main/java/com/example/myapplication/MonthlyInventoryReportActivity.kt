...
class MonthlyInventoryReportActivity : AppCompatActivity() {
    ...
    private fun sendInventoryReport(inventoryReport: List<InventoryReport>)
    {
        ...
        val gson = Gson()
        val json = gson.toJson(inventoryReport)
        Log.d("inventory Report JSON", json)

        val call = postApi.sendInventoryReport("Token $token", inventoryReport)

        call.enqueue(object : Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                if (response.isSuccessful) {
                    Toast.makeText(this@MonthlyInventoryReportActivity, "inventory report sent successfully", Toast.LENGTH_SHORT).show()
                } else {
                    val errorBody = response.errorBody()?.string()
                    val statusCode = response.code()
                    println("Error: $statusCode, $errorBody")
                    Toast.makeText(this@MonthlyInventoryReportActivity, "Failed to send inventory report", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                Toast.makeText(this@MonthlyInventoryReportActivity, "Error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
